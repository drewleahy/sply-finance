        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {thesisPoints.map((point, index) => (
            <motion.div
              ...
              className="bg-luxon-navy p-8 rounded-lg border border-luxon-gold/20"
            >
              <div className="text-4xl mb-4">{point.icon}</div>
              <h3 className="text-xl text-luxon-gold mb-4">{point.title}</h3>
              <p className="text-luxon-muted">{point.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
